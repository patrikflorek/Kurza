...
Kurza is an Android app that turns your phone into a wireless touchpad for your Windows PC using Bluetooth HID (Human Interface Device) protocol.

## Features

- **Wireless Mouse Control**: Use your phone's touchscreen as a touchpad to control your PC cursor
...
- **No Software Required on PC**: Uses standard Bluetooth HID protocol - works natively with Windows

## Requirements

- **Android Phone**: Android 9.0 (API 28) or higher with Bluetooth support
- **Windows PC**: Windows 10/11 with Bluetooth
...
1. **Remove any existing pairing** between your phone and PC:
   - On Windows: Settings → Bluetooth & devices → Find your phone → Remove device
   - On your phone: Bluetooth settings → Forget/Unpair the PC
...
4. **Pair from Windows**:
   ...
   - Windows should recognize it as an input device (mouse)

5. **Connect in the app**:
   - After pairing, your PC will appear in the device list
...
- **Manual**: Open the app and tap **"Reconnect to [PC Name]"** button
...
### Phone shows as "Other device" on Windows
...
The PC can handle multiple Bluetooth connections. Your phone acting as a mouse doesn't interfere with other devices.

## Technical Details

- Uses Android's `BluetoothHidDevice` API (Bluetooth HID Device Profile)
- Phone acts as an HID peripheral (mouse), PC acts as HID host
...
```
app/src/main/java/link/sciber/curza/
...
└── ui/
    └── TouchpadScreen.kt        # Touchpad UI with gesture detection
```
